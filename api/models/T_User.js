...
async function asyncHideDialog(loginId) {
    let user = await asyncGetUserbyLoginId(loginId);
    if (!user) {
    ...
    let queryResult = null;
    let sqlRequest = new sql.Request();
    sqlRequest.input('LoginId', sql.NVarChar, loginId);
    sqlRequest.input('HokenShokenF', sql.Bit, 1);

    let sqlStr = `UPDATE ${T_Table.tableName} SET HokenShokenF = @HokenShokenF WHERE LoginId = @LoginId`;
    let query = new Promise(function (resolve, reject) {
        sqlRequest.query(sqlStr, (err, result) => {
            if(result) {
                resolve(result);
    ...
    await query.then(async function(res) {
        ...
        let logData = [
            { key: "Time", content: new Date() },
            { key: "File", content: "T_User.js" },
            { key: "Function", content: "asyncHideDialog" },
            { key: "Sql", content: sqlStr },
            { key: "Param", content: loginId }
        ]
        await logService.sqlLog(logData);
...
async function searchCustomer(objSearch) {
    ...
    try {
        ...
        let orderStr = await baseModel.buildOrder(objSearch, fieldWhiteList, `UserNo, SeqNo`);
