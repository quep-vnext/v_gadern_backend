...
async function login(loginUser) {
    ...
    if (user.Password === password){
        ...
        let riyo = {
            LoginId: login_id,
            ActionContent: RiyoAction.Login
        }
        RiyoService.createRiyo(riyo);
        
        return {
            code: ResCode.SUCCESS,
            message: 'Login success',
            data: { token: token, user_no: user.UserNo, family_no: user.FamilyNo, agent_cd: user.AgentCd }
        }
    } else {
        return {
            code: ResCode.AUTH_FAIL,
...
async function registerNewUser(userinfo) {
    if(validation.validateEmail(userinfo.email)) {
        ...
        if (createdUser !== ResCode.DUPLICATE && createdUser !== ResCode.NOT_EXIST) {
            //add register action to T_Riyo
            let riyoData = {
                LoginId: userinfo.email,
                ActionContent: RiyoAction.HokenBunseki_Kaishi
            }
            await RiyoService.createRiyo(riyoData);
            let sendUserinfo = {
    ...
    if(validation.validateEmail(email)) {
        ...
        if (resetResult) {
            ...
            let riyo = {
                ...
                ActionContent: RiyoAction.ReSendPass
            }
            RiyoService.createRiyo(riyo);
...
async function asyncHideDialog(loginId) {
    let checkValidation = validation.validateEmail(loginId);
    if (!checkValidation){
        return {
            code: ResCode.REQUIRED,
            message: 'loginId is invalid!'
        }
    }
    let update = await UserModel.asyncHideDialog(loginId);
    if (update === ResCode.NOT_EXIST) {
        return {
            code: ResCode.NOT_EXIST,
...
async function changePassword(objSearch) {
    ...
    if (user.Password === objSearch.oldPass || (typeof process.env.NODE_ENV !== "undefined" && process.env.NODE_ENV.trim() === 'test')){
        ...
        if (result) {
            ...
            let riyo = {
                LoginId: user.LoginId,
                ActionContent: RiyoAction.ChangePass
            }
            RiyoService.createRiyo(riyo);
